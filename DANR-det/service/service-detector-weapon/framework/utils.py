...
def prepared_response_v2(prediction, request_json):
    ...
    for item in request_json['retrieve_source']:
        ...
        violation_data['hits'] = []
        for subtype in item['violation_type']['p40']:
            hit = {
                'dataset_name': item['dataset_name'],
                'probability': None if subtype != 'p401' else '%.3f' % prediction,
                'md5': '',
                'sub_type': subtype}
...
def checkValidImage_CV2(cv2_image, min_h=50, min_w=50):
    """
    check if resolution is too small
    :param image: IPLImage
    :return: Boolean
    """
    image_h, image_w, _ = cv2_image.shape
    return image_w > min_w and image_h > min_h
...
def parse_request(request: flask.request, threshold: float):
  ...
  if len(request_json['retrieve_source']) == 0:
     ...
     for item in request_json['retrieve_source']:
       ...
       '''
       if item['dataset_name'] != 'default':
         return request_json, {'msg': 'dataset_name is not default', 'code': 404}
       '''

       if not isinstance(item['violation_type'], dict):
         return request_json, {'msg': 'violation_type format error (not dict)', 'code': 400}

       if 'p40' in item['violation_type']:
           for cur_type in item['violation_type']['p40']:
               if cur_type not in ["p401", "p402", "p403", "p404", "p405"]:
                   return request_json, {'msg': 'violation_subtype does not match violation_type', 'code': 404}

       if 'p40' in item['violation_type'] or len(item['violation_type']) == 0:
         filter_retrieve_source.append(item)

     if len(filter_retrieve_source) == 0:
       return request_json, {'msg': 'p40 not in retrieve_source', 'code': 400}
